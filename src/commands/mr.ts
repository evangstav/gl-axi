...
async function listMrs(args: string[], ctx: GitLabContext): Promise<string> {
  ...
  if (source) params.set("source_branch", source);
...
async function showMr(args: string[], ctx: GitLabContext): Promise<string> {
  const id = requireMrId(args);
  const mr = await glabApi<Record<string, unknown>>(
    `${mrBase(ctx)}/${id}`,
    ctx,
  );
...
async function createMr(args: string[], ctx: GitLabContext): Promise<string> {
  const source =
    getFlag(args, "--source") ?? getFlag(args, "-s") ?? currentBranch();
  if (!source) {
    throw new AxiError(
...
async function mergeMr(args: string[], ctx: GitLabContext): Promise<string> {
  const id = requireMrId(args);
 ...
 */
async function checksMr(args: string[], ctx: GitLabContext): Promise<string> {
  const id = requireMrId(args);
  const mr = await glabApi<Record<string, unknown>>(
    `${mrBase(ctx)}/${id}`,
    ctx,
  );
  const approvals = await glabApi<Record<string, unknown>>(
    `${mrBase(ctx)}/${id}/approvals`,
    ctx,
  );

  const pipeline =
    (mr["head_pipeline"] as Record<string, unknown> | undefined) ??
    (mr["pipeline"] as Record<string, unknown> | undefined);
  const pipelineStatus = (pipeline?.["status"] as string | undefined) ?? "none";
  const approvalsRequired = Number(approvals?.["approvals_required"] ?? 0);
  const approvalsLeft = Number(approvals?.["approvals_left"] ?? 0);
  const approved = (approvals?.["approved"] as boolean | undefined) ??
    approvalsLeft === 0;
...
async function addReviewer(
  ...
  if (!reviewer) {
    throw new AxiError(
      "--reviewer is required (username or numeric user id)",
      "VALIDATION_ERROR",
    );
  }
  ...
  if (required) {
    ...
    if (userId === undefined) {
      throw new AxiError(
        `Could not resolve "${reviewer}" to a GitLab user for the required-approver rule`,
        "VALIDATION_ERROR",
        [
...
async function listReviewers(
  ...
  const approvedBy = new Set(
    ((approvals?.["approved_by"] as Record<string, unknown>[] | undefined) ?? [])
      .map((a) => (a["user"] as Record<string, unknown> | undefined)?.["username"])
      .filter((u): u is string => typeof u === "string"),
  );
  const reviewers =
    (mr["reviewers"] as Record<string, unknown>[] | undefined) ?? [];
  const rows = reviewers.map((r) => ({
    username: r["username"],
    name: r["name"],
    approved: approvedBy.has(r["username"] as string),
  }));
  return renderOutput([
...
function parseMrIid(output: string): number | undefined {
...
function requireMrId(args: string[], start = 1): number {
  const raw = getPositional(args, start);
  if (!raw || !/^\d+$/.test(raw)) {
    throw new AxiError(
      "A merge request id is required, e.g. gl-axi mr show 42",
      "VALIDATION_ERROR",
    );
  }
  return Number(raw);
