...
export function resolveContext(flagValue?: string): GitLabContext | undefined {
  ...
  const host =
    process.env["GITLAB_HOST"]?.trim() || origin?.host || "gitlab.com";
...
): { projectPath: string; source: GitLabContext["source"] } | undefined {
  const flagPath = normalizePath(flagValue);
  if (flagPath) return { projectPath: flagPath, source: "flag" };

  const envPath = normalizePath(process.env["GL_REPO"]);
  if (envPath) return { projectPath: envPath, source: "env" };

  if (origin) return { projectPath: origin.projectPath, source: "git" };
  return undefined;
}
...
function readOrigin(): Origin | undefined {
  try {
...
export function parseRemoteUrl(url: string): Origin | undefined {
  ...
  if (scpForm) {
    ...
    if (projectPath) return { host: scpForm[1], projectPath };
  }

  return undefined;
}

function stripPath(path: string): string | undefined {
  const segments = path
    .replace(/^\/+|\/+$/g, "")
    .split("/")
    .filter(Boolean)
...
function readToken(baseUrl: string): string | undefined {
  try {
