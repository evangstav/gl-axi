...
const patterns: ErrorPattern[] = [
  ...
  },
  {
    ...
    code: "NOT_FOUND",
    message: (m) =>
      m[1] ? `Merge request !${m[1]} does not exist` : "Merge request not found",
  },
  {
    pattern: /404 Issue Not Found|issue.*?#?(\d+).*?not found/i,
    code: "NOT_FOUND",
    message: (m) =>
      m[1] ? `Issue #${m[1]} does not exist` : "Issue not found",
  },
  {
    // `401` is word-bounded so it matches an HTTP 401 but not codes that merely
    // contain the digits "401".
    pattern:
      /\b401\b|401 Unauthorized|requires authentication|invalid[_ ]token|token is (?:invalid|expired|revoked)|GITLAB_TOKEN/i,
    code: "AUTH_REQUIRED",
    message: () =>
      "GitLab auth failed — token missing, expired, or wrong scope",
    suggestions: () => [
  ...
  },
  {
    ...
    message: () =>
      "Conflict — a matching merge request may already exist for this branch",
...
function redactSensitive(value: string): string {
  return value.replace(
    /\b(password|token|GITLAB_TOKEN|private[_-]token|PRIVATE-TOKEN)[=:]\s*\S+/gi,
    "$1=[redacted]",
  );
}
...
export function mapGlabError(stderr: string, exitCode: number): AxiError {
  ...
  return new AxiError(
    errorExcerpt(stderr) || `glab exited with code ${exitCode}`,
    "UNKNOWN",
  );
}
