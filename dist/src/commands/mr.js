...
async function addReviewer(args, ctx) {
    ...
    if (!username) {
        throw new AxiError(`Could not resolve user id "${reviewer}" to a username`, "VALIDATION_ERROR", ["Pass the reviewer's username instead of a numeric id"]);
    }
    await glabExec(["mr", "update", String(id), "-R", ctx.projectPath, "--reviewer", `+${username}`], ctx);
